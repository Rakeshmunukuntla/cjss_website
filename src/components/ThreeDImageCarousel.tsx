...
@media screen and (min-width: 768px) {
    ...
    .cascade-slider_slides img { max-width: 310px; }
}
@media screen and (min-width: 991px) {
    ...
    .cascade-slider_slides img { max-width: 370px; }
      ...
      >
        <div className="cascade-slider_slides">
          {slides.map((slide, index) => (
            <div
              key={slide.id}
              className={`cascade-slider_item ${getSlideClasses(index, activeIndex, total, itemCount)} relative group`}
            ...
            >
              <div className="relative cursor-pointer">
                ...
                {/* Read More Button on Hover - Only show for center slide */}
                {hoveredSlideId === slide.id && getSlideClasses(index, activeIndex, total, itemCount) === 'now' && (
                  <div className="absolute inset-0 flex items-center justify-center bg-neutral-900/60 rounded-[35px] transition-all duration-300">
                    <button
                      onClick={(e) => {
                        e.preventDefault()
                        if (onReadMore) {
                      ...
                      className="px-6 py-2 bg-purple-500 text-white font-bold rounded-lg transition-colors duration-300 shadow-lg hover:bg-purple-400"
                    >
                      Read More
                    </button>
                  </div>
        ...
        {total > 1 && (
          <>
            <span
              className="cascade-slider_arrow cascade-slider_arrow-left rounded-full bg-neutral-900/70 text-white p-2 hover:bg-purple-500/40 hover:text-white transition-colors duration-300 shadow-lg shadow-purple-500/20"
              onClick={(e) => {
                e.stopPropagation()
                navigate('prev')
              }}
            ...
            <span
              className="cascade-slider_arrow cascade-slider_arrow-right rounded-full bg-neutral-900/70 text-white p-2 hover:bg-purple-500/40 hover:text-white transition-colors duration-300 shadow-lg shadow-purple-500/20"
              onClick={(e) => {
                e.stopPropagation()
                navigate('next')
              }}
