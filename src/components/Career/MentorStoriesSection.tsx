...
//                 .replace(/\\n\\n/g, "\n\n") // convert literal \n\n to real newlines
//                 .replace(/\\n/g, "\n") // convert literal \n to real newlines
//                 .split(/\n\n+/) // split on double (or more) newlines first
//                 .flatMap(
//                   (
//                     block, // then split remaining single \n
//                   ) => block.split(/\n/),
//                 )
...
//               <p className="text-white/50 text-sm">Loading mentor stories...</p>
//             </div>
//           )}
...
//                 className="px-4 py-2 text-xs rounded-full border border-white/20 text-white/60 hover:text-white hover:border-white/40 transition-all"
//               >
//                 Retry
//               </button>
//             </div>
  ...
  useEffect(() => {
    const fetchMentors = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch("https://cjsstech.com/api/mentors/");
        if (!response.ok) {
          throw new Error(`Failed to fetch mentors: ${response.statusText}`);
        }
        const data = await response.json();
